...
export async function POST(request: NextRequest) {
  try {
    console.log('🔍 Test MTN simple');
    
    const testData = {
      operation: 'initiate',
      reference: `MTN${Date.now().toString().slice(-8)}`,
      amount: 100,
      phone: '672886348',
      method: 'mobilemoney',
      country: 'CM',
    ...
    console.log('📤 Données MTN:', JSON.stringify(testData, null, 2));
    ...
    console.log('✅ Réponse MTN:', responseText);
    ...
    return NextResponse.json({
      success: true,
      message: 'Test MTN simple terminé',
      testData: testData,
      ...
      analysis: {
        ...
        recommendation: isSuccess 
          ? '✅ MTN fonctionne correctement'
          : '❌ MTN ne fonctionne pas'
      }
    });
    
  } catch (error) {
    console.error('❌ Erreur test MTN simple:', error);
    return NextResponse.json({
      success: false,
      message: 'Erreur lors du test MTN simple',
