...
function InscriptionForm() {
  ...
  useEffect(() => {
    const code = searchParams.get("parrain");
    if (code) setParrain(code);
  }, [searchParams]);
    ...
    fetch("/api/auth/register", {
      ...
      .then((data) => {
        if (data.error) {
          ...
          // Déclencher un event pour incrémenter le compteur sur la page d'accueil
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-8 w-full max-w-md relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Code de parrainage (optionnel)</label>
            <input
              type="text"
              className="w-full rounded-lg border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 p-2"
